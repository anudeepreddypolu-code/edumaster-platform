...
import { UxFinding } from './types.js';
import { writeText } from './utils.js';

export const buildFlutterCode = (findings: UxFinding[]) => {
  const topRecommendations = findings.slice(0, 4).map((finding) => `// - ${finding.recommendation}`).join('\n');
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 8),
                Text(
                  '${findings.length === 0 ? 'No findings were passed into the generator.' : ''}',
                  style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: const Color(0xFF62748A)),
                ),
              ],
            ),
...
export const writeFlutterFile = async (outputDir: string, findings: UxFinding[]) => {
  const filePath = path.join(outputDir, 'improved_ui.dart');
  await writeText(filePath, buildFlutterCode(findings));
  return filePath;
};

if (process.argv[1]?.endsWith('flutter-generator.ts')) {
  const fallbackDir = process.argv[2] || path.resolve(process.cwd(), 'qa-automation', 'flutter_reference', 'lib');
  writeFlutterFile(fallbackDir, []).then((filePath) => {
    console.log(`Flutter reference written to ${filePath}`);
  });
}
